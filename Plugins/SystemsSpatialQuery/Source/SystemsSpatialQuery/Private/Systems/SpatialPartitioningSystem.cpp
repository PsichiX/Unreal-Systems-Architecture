...
void SpatialPartitioningSystem(USystemsWorld& Systems)
{
	...
	if (Settings->bRebuildOnlyOnChange &&
		Systems.ResourceDidChanged<USpatialPartitioning>() == false &&
	...
	{
		Partitioning->Reset(Settings->CoverWorldArea,
			Settings->CellActorsCapacity,
			Settings->PreferedSubdivisionPlane);
...
void DebugSpatialPartitioningSystem(USystemsWorld& Systems)
{
	...
	if (IsValid(Settings->StressTestActor.Get()))
	{
		...
		auto CurrentCount = Systems.Actors()
								.Filter(
									[&](const auto* Actor) {
										return Actor->GetClass() ==
											Settings->StressTestActor.Get();
									})
								.Count();
		...
		while (CurrentCount < Settings->StressTestSpawnCount)
		{
			++CurrentCount;
			auto Position =
				FVector(FMath::FRandRange(Area.Lower.X, Area.Upper.X),
				...
				if (bOccupied)
				{
					if (Settings->bDrawOccupiedDebugAreas)
					{
						DrawDebugBox(World,
							Area.Center(),
							Area.Extents(),
							Settings->DebugOccupiedAreasColor);
					}
				}
				else
				{
	...
	{
		const auto Epicenter =
			Systems.Query<UDebugSpatialQueryEpicenterComponent>().Next();

		if (Epicenter.IsSet())
		{
			const auto Position =
				Epicenter.GetValue().Get<0>()->GetActorLocation();
			const auto Mode = Epicenter.GetValue().Get<1>()->Mode;
			auto Query =
				Partitioning->Query<USpatialComponent>(Systems, Position);
			if (Mode == EDebugSpatialQueryEpicenterMode::Area)
			{
				...
				DrawDebugBox(World,
					Area.Center(),
					Area.Extents(),
					Settings->DebugOrderedActorsColor);
				Query = Partitioning->QueryInArea<USpatialComponent>(
					Systems, Position, Area);
			}
			else if (Mode == EDebugSpatialQueryEpicenterMode::Radius)
			{
				...
				DrawDebugSphere(World,
					...
					32,
					Settings->DebugOrderedActorsColor);
				Query = Partitioning->QueryInRadius<USpatialComponent>(
					Systems, Position, Radius);
			}
			...
			Query.Enumerate().ForEach(
				[&](const auto EnumeratedQueryItem)
				{
					...
					const auto Content =
						FString::Printf(TEXT("#%u (%f)"), Index, Distance);
